...
void sys_sendevent(int eventName, int targetClass) {
eventManager.sendEventTo(eventName, targetClass);
}
...
#define WHITE 520
#define GRAY 490
#define GRAY_MAX (GRAY + 0)
#define GRAY_MIN (GRAY - 10)
#define BLACK 260
#define OFFSET 60
#define EDGE_COLOR ((GRAY + WHITE) / 2) - OFFSET
...
void TaskEvent(VP_INT exinf)
{
 ...
 if(gray_count > 2) {
 gray_count = 0;
 eventManager.sendEvent(EVENT_gray);
...
// generate_external_TouchNotPress(NULL);
  eventManager.sendEvent(EVENT_release);
 }
...
/// lcd.putf("s", "press");
 ...
 {
  eventManager.sendEvent(EVENT_touch2);
...
// lcd.putf("s", "not press");
 ...
 {
  eventManager.sendEvent(EVENT_release2);
 }
 ...
 {
  eventManager.sendEvent(EVENT_sonar);
 lcd.putf("s", "Sonar\n");
 }
